...
`QUICKBOOKS_REALM_ID` is optional for the initial `npm run auth` bootstrap flow. It will be written to `.env` after a successful OAuth callback.
...
Run the compiled server with `MCP_TRANSPORT=streamable-http` to expose:
...
The repo includes Vercel API functions and rewrites so deployed MCP clients can use:
...
`npm run auth` is still the local bootstrap flow. It opens a browser, uses the redirect URI configured in your Intuit app, and writes refreshed tokens plus the discovered realm ID back to your local `.env` file. That flow is for local development only, not for deployed Vercel functions.
